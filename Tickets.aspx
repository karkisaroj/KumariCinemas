...
<head runat="server">
    ...
    <style>
        ...
        .table tbody tr:hover{background-color:#f5f5f5}.table td{vertical-align:middle;font-size:.88rem;padding:11px 16px}
        ...
        .btn-cancel-ticket{background:#fce4ec;color:#c62828;border:none;border-radius:6px;padding:5px 11px;font-size:.79rem}.btn-cancel-ticket:hover{background:#ef9a9a}
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-card">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div><h4 class="page-title mb-0"><i class="bi bi-ticket-perforated me-2"></i>Tickets</h4><small class="text-muted">Manage all ticket bookings</small></div>
            <asp:Button ID="btnShowAdd" runat="server" Text="+ Book Ticket" CssClass="btn btn-add" OnClick="btnShowAdd_Click"/>
        </div>
        <div class="search-wrapper mb-3"><i class="bi bi-search"></i><input type="text" id="searchBox" class="form-control search-input" placeholder="Search tickets..." onkeyup="filterTable()"/></div>
        <div class="table-responsive">
            <asp:GridView ID="gvTickets" runat="server" CssClass="table table-hover align-middle mb-0"
                ...
                OnRowDeleting="gvTickets_RowDeleting" OnRowCommand="gvTickets_RowCommand"
                OnRowEditing="gvTickets_RowEditing" OnRowCancelingEdit="gvTickets_RowCancelingEdit">
                <Columns>
                    <asp:TemplateField HeaderText="ID"><ItemTemplate><span class="badge-id">#<%# Eval("TICKET_ID") %></span></ItemTemplate></asp:TemplateField>
                    <asp:TemplateField HeaderText="Customer"><ItemTemplate><div class="fw-semibold"><%# Eval("CUSTOMER_NAME") %></div></ItemTemplate></asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="Status">
                        <ItemTemplate>
                            <span class='<%# Eval("TICKET_STATUS").ToString()=="Booked"?"badge-booked":Eval("TICKET_STATUS").ToString()=="Cancelled"?"badge-cancelled":"badge-used" %>'><%# Eval("TICKET_STATUS") %></span>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="Actions">
                        <ItemTemplate>
                            <asp:Button runat="server" Text="Cancel" CssClass="btn btn-cancel-ticket"
                            ...
                            <asp:Button runat="server" Text="Delete" CssClass="btn btn-delete-row ms-1"
                                CommandName="Delete" OnClientClick="return confirm('Delete this ticket?');"/>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
                <EmptyDataTemplate><div class="text-center text-muted py-5">No tickets found.</div></EmptyDataTemplate>
            </asp:GridView>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6 mb-3"><label class="form-label">Customer</label><asp:DropDownList ID="ddlCustomer" runat="server" CssClass="form-select"></asp:DropDownList></div>
                    <div class="col-md-6 mb-3"><label class="form-label">Movie</label><asp:DropDownList ID="ddlMovie" runat="server" CssClass="form-select"></asp:DropDownList></div>
                    <div class="col-md-6 mb-3"><label class="form-label">Theater</label><asp:DropDownList ID="ddlTheater" runat="server" CssClass="form-select"></asp:DropDownList></div>
                    <div class="col-md-6 mb-3"><label class="form-label">Hall</label><asp:DropDownList ID="ddlHall" runat="server" CssClass="form-select"></asp:DropDownList></div>
                    <div class="col-md-6 mb-3"><label class="form-label">Showtime</label><asp:DropDownList ID="ddlShowtime" runat="server" CssClass="form-select"></asp:DropDownList></div>
                    <div class="col-md-3 mb-3"><label class="form-label">Seat No</label><asp:TextBox ID="txtSeat" runat="server" CssClass="form-control" placeholder="e.g. A12"></asp:TextBox></div>
                    <div class="col-md-3 mb-3"><label class="form-label">Price (Rs.)</label><asp:TextBox ID="txtPrice" runat="server" CssClass="form-control" TextMode="Number" placeholder="e.g. 500"></asp:TextBox></div>
                </div>
            </div>
...
<script>
    ...
    window.onload = function () {
        ...
        if (<%= ShowModal.ToString().ToLower() %>) { new bootstrap.Modal(document.getElementById('ticketModal')).show(); }
    };
</script>
</body></html>